...
void triangle(
	float xCoin1,float yCoin1,
	float xCoin2,float yCoin2,
	...
	if(filled){
		...
		ligne(xCoin2,yCoin2, xCoin3,yCoin3);
		ligne(xCoin3,yCoin3, xCoin1,yCoin1);
	}
}
...
void rectangle(
	...
	if(filled){
		...
		ligne(xCoin2,yCoin1, xCoin2,yCoin2);
		ligne(xCoin2,yCoin2, xCoin1,yCoin2);
		ligne(xCoin1,yCoin2, xCoin1,yCoin1);
	}
}
...
/* Dessine une ligne de couleur courante aux coordonnees donnees */
void ligneRGB(
	float xDepart,float yDepart,
	float xArrivee,float yArrivee,
	int r,int g,int b
){
	glColor3f(r/255.f, g/255.f, b/255.f);
	glBegin(GL_LINES);
...
/* Dessine un rectangle de couleur (rouge,vert,bleu) aux coordonnees donnees */
void rectangleRGB(
	float xCoin1,float yCoin1,
	float xCoin2,float yCoin2,
	int filled,
...
img readRGBA2ARVB(char name[100], img image){
	...
	char* temp = NULL;
	if(file != NULL)
		temp = fgets(data, (int)(3*4*image.height*image.width+1) ,file);
	fclose(file);
	temp = temp; //just to shut up compilator warnings (temp is not used !)

	unsigned char* intData = malloc(4*image.width*image.height);
	int intData_index = 0;
	//temp color components
	unsigned char red = 0;
	unsigned char green = 0;
	unsigned char blue = 0;
	unsigned char alpha = 0;
	//for each pixel
	const int MAX_PX_IN_IMG = (int)(image.width*image.height-1);
	for(int px_index=MAX_PX_IN_IMG; px_index >= 0; px_index--){ //for each pixel
		red = (unsigned char)(
			100*charToInt(data[ (int)(3*4*px_index  ) ]) +
			 10*charToInt(data[ (int)(3*4*px_index+1) ]) +
				charToInt(data[ (int)(3*4*px_index+2) ])
		);
		green = (unsigned char)(
			100*charToInt(data[ (int)(3*4*px_index+3) ]) +
			 10*charToInt(data[ (int)(3*4*px_index+4) ]) +
				charToInt(data[ (int)(3*4*px_index+5) ])
		);
		blue = (unsigned char)(
			100*charToInt(data[ (int)(3*4*px_index+6) ]) +
			 10*charToInt(data[ (int)(3*4*px_index+7) ]) +
				charToInt(data[ (int)(3*4*px_index+8) ])
		);
		alpha = (unsigned char)(
			100*charToInt(data[ (int)(3*4*px_index+9 ) ]) +
			 10*charToInt(data[ (int)(3*4*px_index+10) ]) +
				charToInt(data[ (int)(3*4*px_index+11) ])
		);
		intData[intData_index  ] = red;
		intData[intData_index+1] = green;
		intData[intData_index+2] = blue;
		intData[intData_index+3] = alpha;
		intData_index += 4;
	}
	image.data = intData;
	return image;
